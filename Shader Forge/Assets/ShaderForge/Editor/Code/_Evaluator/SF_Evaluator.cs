...
namespace ShaderForge
{
    ...
    public class SF_Evaluator
    {
        ...
        private void PropertiesShaderLab()
        {
            BeginProperties();
        ...
        private static string WrapInIfndef(string def)
        {
            if (!def.Contains("#ifndef"))
            {
                string ifndef = "#ifndef " + def.Replace("#define ", "") + "\n   ";
                ifndef += def;
                ifndef += "\n#endif";
                return ifndef;
            }
        ...
        private void BeginCG()
        {
            ...
            switch (currentPass)
            {
                case PassType.FwdBase:
                    App(WrapInIfndef("#define UNITY_PASS_FORWARDBASE"));
                    break;
                ...
                case PassType.Deferred:
                    App(WrapInIfndef("#define UNITY_PASS_DEFERRED"));
                    break;

                case PassType.ShadCast:
                    App(WrapInIfndef("#define UNITY_PASS_SHADOWCASTER"));
                    break;
        ...
        private string LightmapNormalDir()
        {
        ...
        private void PrepareLightmapVars()
        {
            if (!LightmapThisPass())
                return;
        ...
        private void InitAttenuation()
        {
            ...
            string src = (currentProgram == ShaderProgram.Frag) ? "i" : "o";

            string atten = "UNITY_LIGHT_ATTENUATION(attenuation, " + src + ", " + src + ".posWorld.xyz)";
            string inner = (ShouldUseLightMacros() ? atten : "float attenuation = 1");
            App(inner + ";");
            if (ps.catLighting.lightMode != SFPSC_Lighting.LightMode.Unlit)
                App("float3 attenColor = attenuation * _LightColor0.xyz;");
        }

        private string GetWithDiffPow(string s)
        {
        ...
        private void CalcDiffuse()
        {
            ...
            if (!InDeferredPass())
            {
                ...
                if (ps.HasTransmission() || ps.HasLightWrapping())
                {
                    ...
                    const string fwdLight = "float3 forwardLight = "; // TODO
                    const string backLight = "float3 backLight = "; // TODO
                ...
                if (Unity5PBL())
                {
                    ...
                    const string pbrStr = "((1 +(fd90 - 1)*nlPow5) * (1 + (fd90 - 1)*nvPow5) * NdotL)";
        ...
        private void InitNormalDirFrag()
        {
		...
		*/
        ...
        private void AppFinalOutput(string color, string alpha)
        {
            string rgbaValue;
        ...
        private string GetFogColorAsFixed3Value()
        {
            Color c = ps.catBlending.fogColor;
            return "fixed4(" + c.r + "," + c.g + "," + c.b + "," + c.a + ")";
        }

        private string GetAmbientStr()
        {
            string s;
            if (InDeferredPass())
                s = "unity_Ambient.rgb";
        ...
        private string SumString(bool[] bools, string[] strings, string defStr)
        {
            int validCount = 0;
        ...
        private void InitReflectionDir()
        {
        ...
        private void InitSceneColorAndDepth()
        {
        ...
        private void VertexInputStruct()
        {
        ...
        private void TransferCommonData()
        {
        ...
        private void VertexOutputStruct()
        {
        ...
        {
            return LightmappedAndLit() || IsShadowPass() || (cNodes.Count(x => x is SFN_LightAttenuation) > 0);
        }
...
*/
        ...
        private void DeferredFragReturn()
        {
            // DIFFUSE
            if (ps.HasDiffuse())
            {
        ...
        private void LightmapMetaPassFrag()
        {
        ...
        private void TessellationPatchConstant()
        {
        ...
        private void TessellationVertexTransfer()
        {
        ...
        private void TessellationHullConstant()
        {
        ...
        private void TessellationDomain()
        {
            App("[domain(\"tri\")]");
        ...
        private void TransferBarycentric(string s)
        {
            App("v." + s + " = vi[0]." + s + "*bary.x + vi[1]." + s + "*bary.y + vi[2]." + s + "*bary.z;");
        }

        private void FuncTessellation()
        {
        ...
        private void CheckClip()
        {
            ...
            {
                string ditherStr = SFPSC_Blending.strDithering[(int)ps.catBlending.dithering].Split(' ')[0];
        ...
        private void WriteCustomEditor()
        {
